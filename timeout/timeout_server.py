...
from timeout.crypto_utils import CryptoManager
...
def safe_register(nickname: str, conn: socket.socket, crypto: CryptoManager) -> str:
    ...
    with clients_lock:
        ...
        clients[candidate] = {
            ...
            'last_activity': current_time,  # 最後 TCP 活動時間
            'idle_warned': False  # 是否已發送閒置警告
        }
    return candidate
        ...
        while True:
            ...
            with clients_lock:
                if nickname in clients:
                    ...
                    clients[nickname]['idle_warned'] = False  # 重置警告狀態
