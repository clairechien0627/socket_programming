...
class SecureChatGUI:
    ...
    def on_send_file(self) -> None:
        ...
        try:
            ...
            # 顯示開始傳送
            self.append_text(f"[system] 📤 正在發送檔案: {filename} ({filesize} bytes)\n")
            ...
            self.append_text(f"[system] ❌ 檔案選擇失敗: {e}\n")
    
    def send_file_thread(self, filepath: str, filename: str, filesize: int) -> None:
        """在背景執行緒中傳送檔案"""
        file_sock = None
        try:
            # 連線到檔案傳輸伺服器
            file_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            file_sock.connect((self.host, FILE_PORT))
            ...
            metadata = {
                ...
                'filename': filename,
                'filesize': filesize,
                'recipient': 'all'  # 廣播給所有人
            }
            send_message(file_sock, json.dumps(metadata))
            ...
            with open(filepath, 'rb') as f:
                ...
                while total_sent < filesize:
                    ...
                    if progress % 20 == 0:  # 每 20% 更新一次
                        self.root.after(0, self.append_text, 
                                      f"[system] 📊 傳送進度: {progress}%\n")
                    
                    # 模擬網路延遲
                    time.sleep(0.01)
            
            # 接收伺服器回應
            response_json = recv_message(file_sock)
            if response_json:
                response = json.loads(response_json)
                if response.get('status') == 'success':
                    self.root.after(0, self.append_text,
                                  f"[system] ✅ 檔案發送成功: {filename}\n")
                else:
                    self.root.after(0, self.append_text,
                                  f"[system] ❌ 檔案發送失敗: {response.get('message')}\n")
